              ...
              <div class="modal-content">
                  <form id="detailsForm500" method="post" action="{{ route('krabulkUpload') }}"
                      ...
                      <div class="modal-body">
                          ...
                          <div class="form-group" id="mapped-section1">
                              <label><b>Are you want to mapped common KRA:</b></label>
                              <select name="kratemplatesId1" id="kratemplatesdata114" class="form-control">
                                  <option value="">Please Select One</option>
                              </select>
                          </div>
                          ...
                          <div class="form-group">
                              ...
                              <input type="hidden" class="form-control" name="templatejson114" id="templatejson114"
                                  value="">
                          </div>

                          <div class="container-confirm114">
                              <div class="form-group">
                                  <span><b>Do you want to save as KRAs template ? </b></span>
                                  <input type="checkbox" id="enabletemplate114" style="margin: 5px;"
                                      title="You want to save as KRAs template then please click on the checkbox">
                              </div>
                              <div class="form-group" id="templateFieldWrapper114"></div>
                          </div>
          ...
          <div class="modal-dialog modal-lg">
              <div class="modal-content">
                  <form id="detailsForm" method="post" action="{{ url('kras/excelupload/teamwise') }}"
                      ...
                      <div class="modal-header" style="background: #37A000">
                          <h5 style="color:white;" class="modal-title font-weight-600" id="exampleModalLabel4">
                              Team wise Upload Excel to Key Responsibility Areas
                          </h5>
                          <button id="refreshButtonexcel" type="button" class="close" data-dismiss="modal"
                              aria-label="Close">
                              <span aria-hidden="true">&times;</span>
  ...
  <script>
      $(document).ready(function() {
          ...
          $('#kratemplatesdata114').on('change', function() {
              const kratemplatesdataId = $(this).val();
              const templates = JSON.parse($('#krasupload500 input[name="templatejson114"]').val());
              const selectedTemplate = templates.find(t => t.id == kratemplatesdataId);
              const rowsPerPage = 10;
              let currentPage = 1;
              //   alert(templates);
              ...
              function renderTableBody(data, headers, page, perPage) {
                  ...
                  rows.forEach(row => {
                      const rowHtml = `<tr>${headers.map(header =>
                    `<td>${row[header] || ''}</td>`).join('')}</tr>`;
                      $('#table-body1').append(rowHtml);
                  });
              }

              function renderPagination(totalItems, currentPage, perPage, data, headers) {
                  const totalPages = Math.ceil(totalItems / perPage);
                  const pagination = $('#pagination1');
                  pagination.empty();
              ...
              function clearTable() {
                  $('#table-header1, #table-body1, #pagination1').empty();
              }
          });
      ...
      $(document).on('click', '.uploadkrasModel500', function() {
          ...
          const templates = $(this).data('templates');
          const $select = $('#kratemplatesdata114');
          ...
          $('#krasupload500 input[name="templatejson114"]').val(JSON.stringify(templates));
      });
  </script>
  ...
  <script>
      $(document).ready(function() {
          $('#enabletemplate114').on('change', function() {
              const wrapper = $('#templateFieldWrapper114');
          ...
          $('#enabletemplate1').on('change', function() {
              ...
              if ($(this).is(':checked')) {
                ...
                <div>
                    <label><b>Template Name: *</b></label>
                    <input required type="text" class="form-control" name="templatename" placeholder="Enter template name">
                </div>
            `;
                  wrapper.html(inputHtml);
              } else {
