...
class BackEndController extends Controller
{
  ...
  public function index()
  {
    ...
    if (auth()->user()->role_id == 11) {
      ...
      // dd($delayedAssignments);

      //   $delayedAssignments1 = Assignmentmapping::where(function ($q) {
      ...
      //         ->where(function ($inner) {
      //           $inner->whereNull('percentclosedate') // Documentation < 100%
      //             ->orWhereRaw('draftreportdate > tentativeenddate');
      //         });
      //     });

      //     // hare Worked hours greter tha esthour 
      //     $q->orWhereRaw('(
      //     SELECT COALESCE(SUM(totalhour), 0) 
      //     FROM timesheetusers 
      //     WHERE assignmentgenerate_id = assignmentmappings.assignmentgenerate_id
  ...
  public function filterdashboardreport(Request $request)
  {
    ...
    if (auth()->user()->role_id == 11) {
      ...
      // $delayedAssignments = Assignmentmapping::where(function ($q) use ($startDateFormatted, $endDateFormatted, $monthsdigit) {
