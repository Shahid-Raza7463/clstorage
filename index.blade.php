    ...
    <div class="content-header row align-items-center m-0">
        <nav aria-label="breadcrumb" class="col-sm-5 order-sm-last mb-3 mb-sm-0 p-0 ">
            <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
                ...
                {{-- Tuesday closed, client said user can submit timesheet on monday and if not submit then timesheet will be block  --}}
                @if ($diff_in_days >= 15)
                    {{-- if no any timesheetrequest then goes hare --}}
                    @if ($timesheetrequest == null)
                        <li class="breadcrumb-item"><a data-toggle="modal" data-target="#exampleModal21">Add Timesheet
                                Request</a> </li>
                    @else
                        {{-- user submit timesheet request after that $timesheetrequest->validate is not expire then goes hare  --}}
                        ...
                        @elseif ($currentdate > $timesheetrequest->validate && $timesheetrequest->validate != null)
                            {{-- user submit timesheet request after that $timesheetrequest->validate is  expire then goes hare  --}}
                        ...
                        @else
                            @if ($timesheetrequest && $timesheetrequest->status == 0)
                                {{-- user submit timesheet request after that timesheet request is pending then goes hare   --}}
                                @php
