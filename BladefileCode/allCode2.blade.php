    ...
    <div class="card-header">
        <div class="row" style="border: 2px solid gray;">
            <div class="col-xl-12 col-lg-12 col-md-6 col-sm-6">
                <div class="card card-stats statistic-box mb-2 mt-2">
                    <div
                        ...
                        <h3 class="card-title fs-18 font-weight-bold">
                            {{ $checkinreportlisttoday->Total_Checkin_minus_holiday_count }}</h3>
                    </div>
                </div>
            </div>
            @php
                $checkinCards = [
                    ...
                    [
                        'containerStyle' => 'height: 165px; margin-bottom: 18px;',
                        'gradient' => 'linear-gradient(to right, #34e537, rgb(0 86 255))',
                        'icon' => 'fas fa-home',
                        'title' => 'Before 10:30',
            ...
            @foreach ($checkinCards as $checkinCard)
                <div class="col-xl-4 col-lg-6 col-md-6 col-sm-6" style="{{ $checkinCard['containerStyle'] }};">
...
{{--  Start Hare --}}
<td class="textfixed" data-order="{{ strtotime($taskData->created_at) }}">
    {{ date('F d,Y', strtotime($taskData->created_at)) }}</td>


<td class="textfixed" data-sort="{{ strtotime($taskData->created_at) }}">
    {{ date('F d, Y', strtotime($taskData->created_at)) }}
</td>

<td class="textfixed" data-order="{{ date('Y-m-d', strtotime($taskData->created_at)) }}">
    {{ date('F d, Y', strtotime($taskData->created_at)) }}
</td>
{{--  Start Hare --}}
@php
    $section = request()->get('section', 'mycheckin');
@endphp
{{ $section == 'mycheckin' ? 'active' : '' }}

{{ $section == 'teamcheckin' ? 'active' : '' }}
...
<li class="nav-item">
    <a class="nav-link {{ $section == 'mycheckin' ? 'active' : '' }}" id="pills-home-tab" data-toggle="pill"
        href="#pills-home" role="tab" aria-controls="pills-home"
        aria-selected="{{ $section == 'mycheckin' ? 'true' : 'false' }}">
        My Checkin
    </a>
</li>
...
@if (Auth::user()->role_id == 13 || Auth::user()->role_id == 14)
    <li class="nav-item">
        <a class="nav-link {{ $section == 'teamcheckin' ? 'active' : '' }}" id="pills-teamcheckin-tab"
            data-toggle="pill" href="#pills-teamcheckin" role="tab" aria-controls="pills-teamcheckin"
            aria-selected="{{ $section == 'teamcheckin' ? 'true' : 'false' }}">
            Team Checkin
        </a>
    </li>
@endif
...
@if (request()->has('year'))
    <a href="{{ url('totalworkingdays', auth()->user()->teammember_id) }}"
        ...
        <img src="{{ url('backEnd/image/reload.png') }}" style="width: 30px; height: 30px;" alt="Reload">
    </a>
@endif

@if (Request::query('year'))
    <a href="{{ url('totalworkingdays', auth()->user()->teammember_id) }}" style="margin-left:10px; margin-top:1px;">
        <img src="{{ url('backEnd/image/reload.png') }}" style="width:30px; height:30px;" alt="Reload">
    </a>
@endif
...
{{--  Start Hare --}}
href="{{ url('checkinreport/' . $currentDate . '/' . $checkinCard['title']) }}"> --}}
                ...
                <a href="{{ route('billspendingforcollection', $queryParams) }}">
                    ...
                    <a
                        ...
                        <a
                            ...
                            (Note: This is a pseudo-code; Laravel doesn't support this out-of-the-box but it's
                            achievable
